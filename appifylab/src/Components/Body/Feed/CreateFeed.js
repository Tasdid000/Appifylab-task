...
const CreateFeed = () => {
  ...
  const { userData: user, loading: userLoading } = useUserProfile();
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('privacy', 'PUBLIC'); // Always public

    try {
      await axios.post('http://127.0.0.1:8000/apiv1/user/feeds/', formData, {
        headers: {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="_feed_inner_text_area _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
          <form onSubmit={handleSubmit}>
              ...
              <div className="_feed_inner_text_area_box_image">
                {user?.data.image ? (
                  <img
                    ...
                    alt="Profile"
                    className="_txt_img rounded-full object-cover w-12 h-12 border-2 border-white shadow-md"
                  />
                ) : (
            ...
            {/* Media Preview */}
            {(image || video) && (
              <div className="mt-4 -mx-6 px-6 pb-4">
                {image && (
                  <div className="relative inline-block mb-3">
                    <img src={URL.createObjectURL(image)} alt="Preview" className="max-h-96 w-full object-cover rounded-2xl shadow-lg" />
                    <button type="button" onClick={() => setImage(null)} className="absolute top-2 right-2 bg-red-600 text-white rounded-full p-2 hover:bg-red-700">
                      X
                    </button>
                  </div>
                )}
                {video && (
                  <div className="relative inline-block">
                    <video controls className="max-h-96 w-full rounded-2xl shadow-lg">
                      <source src={URL.createObjectURL(video)} />
                    </video>
                    <button type="button" onClick={() => setVideo(null)} className="absolute top-2 right-2 bg-red-600 text-white rounded-full p-2 hover:bg-red-700">
                      X
                    </button>
                  </div>
                )}
            ...
            <div className="_feed_inner_text_area_bottom">
              <div className="_feed_inner_text_area_item">
                ...
                <div className="_feed_inner_text_area_bottom_photo _feed_common">
                  <label className="_feed_inner_text_area_bottom_photo_link cursor-pointer">
                    <input type="file" accept="image/*" onChange={(e) => e.target.files[0] && setImage(e.target.files[0])} className="hidden" />
                ...
                <div className="_feed_inner_text_area_bottom_video _feed_common">
                  <label className="_feed_inner_text_area_bottom_photo_link cursor-pointer">
                    <input type="file" accept="video/*" onChange={(e) => e.target.files[0] && setVideo(e.target.files[0])} className="hidden" />
                    <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">
              ...
              <div className="_feed_inner_text_area_btn">
                <button
                  ...
                  className={`_feed_inner_text_area_btn_link ${loading || !content.trim() ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
